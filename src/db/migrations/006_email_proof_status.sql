-- Migration 006: Add email_proof_status and email_proof_attempted_at to gh_candidate_applications
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'gh_candidate_applications' AND column_name = 'email_proof_status'
  ) THEN
    ALTER TABLE gh_candidate_applications ADD COLUMN email_proof_status TEXT;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'gh_candidate_applications' AND column_name = 'email_proof_attempted_at'
  ) THEN
    ALTER TABLE gh_candidate_applications ADD COLUMN email_proof_attempted_at TIMESTAMPTZ;
  END IF;
END $$;
