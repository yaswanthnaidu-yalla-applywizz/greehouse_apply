...
**Third instance (resolver, later the same day):** `alignToOption` substring-matched then defaulted to `options[0]` whenever the LLM did not return a listed value. A miss became a confident wrong dropdown answer written to `candidate_qa_bank`. Replaced with exact match only; unmatched or confidence `< 0.65` stays `unresolved` (`31b830e`).
