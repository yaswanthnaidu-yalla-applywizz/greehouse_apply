...
```
READY_FOR_REVIEW → APPROVED → APPLYING → APPLIED
                                        → FAILED
                                        → CAPTCHA_REQUIRED
...
- **Pinned demo job queue:** `AWL-31428` / `AWL-YASWANTH` jobs come from bundled fixtures (`akshithaDemoFixtures.ts`) merged with Supabase rows on `GET /api/candidates/:id/jobs` for admins — partial DB rows alone must not hide fixture jobs.
...
- **Email proof (confirmation) — scope before capture:** `queryZohoConfirmationEmail` in `zoho-connector.ts` accepts a row only if ALL hold: subject does **not** match `/security code|\botp\b|one[-\s]?time (pass)?code/i` (rejected rows log `Rejected email (OTP/security code): {subject}`), sender contains `greenhouse-mail.io`, subject matches `/thank you|application received|application confirmed/i`, `received_time >= submitted_at` (never before — the OTP mail precedes the submit), `received_time <= submitted_at + 10min` (`WINDOW_MS` mirrors `emailProofPoller`'s 10min retry budget — change both together), and the existing company match. On success: `Proof email captured: {subject} from {sender} at {timestamp}`. `emailProofPoller.ts` only queries while status is `EMAIL_PROOF_PENDING` — it skips the cycle during `OTP_REQUIRED` / `CAPTCHA_*` so OTP mail can never be captured as proof.
- **An in-flight application is never requeued:** `QUEUED`, `APPLYING`, `OTP_REQUIRED` and `CAPTCHA_REQUIRED` all mean a worker owns the application. `PATCH /api/applications/:id/status` translates an incoming `APPLYING` to `QUEUED` (the submit path) **only** when the current status is outside that set — the dashboard badge polls every 2s and echoes the polled status back, so an unguarded rewrite requeues a submission that is still running. `SubmitterPool` holds `inFlightApplicationIds` as the second guard: a dequeued id already in the set is logged and dropped rather than assigned to a second lane. There is **no automatic requeue on failure** — only an operator submit (`POST /:id/submit` → `enqueueForSubmission`) moves an application to `QUEUED`.
...
5. **Supabase service key only** — no RLS enforced yet (V3 scope)
